...
<td><c:out value="${hr.department}" /></td> 
<td><c:out value="${hr.location}" /></td> 
<td><c:out value="${hr.email}" /></td> 
<td><c:out value="${hr.contact}" /></td> 

<c:if test="${hr.active==true}">
<td bgcolor="green">ACTIVE</td>
</c:if>
<c:if test="${hr.active==false}">
<td bgcolor="red">INACTIVE</td>
</c:if>

<td>
<a href='<c:url value="setstatus?id=${hr.id}&status=true"></c:url>'>APPROVE</a>
<a href='<c:url value="setstatus?id=${hr.id}&status=false"></c:url>'>REJECT</a>
</td>
